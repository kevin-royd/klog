...
// NewPodInformer 创建 Pod 信息通知器
func NewPodInformer(kubeconfig, namespace string) *PodInformer {
	client, err := NewClient(kubeconfig, namespace)
	if err != nil {
		// In a real application, you might want to log this error or return it.
		// For this context, returning nil is the closest match to the provided snippet.
		return nil
	}

	// The factory is created here, but the informer itself is not yet started.
	// The resyncPeriod will be set later, and the factory will be started in the Start method.
	...
	return &PodInformer{
		...
		resyncPeriod: 30 * time.Second, // Default resync period
		namespace:    namespace,
	}
}
